...
{
	gnutls_datum_t *ocsp = ptr;
	unsigned idx = cinfo->cert_index;

	assert(ptr != NULL);

	if (idx > 0) {
		fail("asked for cert index %d, have only %d\n", idx, 0);
	}
	...
	ocsp_response->data = gnutls_malloc(ocsp->size);
	assert(ocsp_response->data != NULL);

	memcpy(ocsp_response->data, ocsp->data, ocsp->size);
	ocsp_response->size = ocsp->size;
...
void doit(void)
{
	...
	/* set OCSP response1 */
	ret = gnutls_certificate_set_ocsp_status_request_function3(clicred, index2, ocsp_func, &ocsp_resp2, 0);
	if (ret < 0)
		fail("ocsp file set failed: %s\n", gnutls_strerror(ret));

	/* set OCSP response2 */
	ret = gnutls_certificate_set_ocsp_status_request_function3(clicred, index1, ocsp_func, &ocsp_resp1, 0);
	if (ret < 0)
		fail("ocsp file set failed: %s\n", gnutls_strerror(ret));
