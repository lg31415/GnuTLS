...
static int ocsp_func(gnutls_session_t session, const gnutls_cert_info_st *cinfo,
		     void *ptr, gnutls_datum_t *response)
{
	response->size = OCSP_SIZE;
	response->data = gnutls_malloc(response->size);
	assert(response->data != NULL);

	memcpy(response->data, OCSP_DATA, response->size);
...
static void start(const char *prio)
{
	...
	gnutls_certificate_set_retrieve_function2(scred,
						  server_cert_callback);

	assert(gnutls_certificate_set_ocsp_status_request_function3(scred, 0,
		ocsp_func, NULL, 0) == GNUTLS_E_REQUESTED_DATA_NOT_AVAILABLE);

	assert(gnutls_certificate_set_ocsp_status_request_function3(scred, 0,
		ocsp_func, NULL, GNUTLS_OCSP_CB_GLOBAL_SET) == 0);
