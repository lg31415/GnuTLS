...
static void server(int fd, const char *prio)
{
	...
	gnutls_certificate_credentials_t x509_cred;
	gnutls_range_st range, cur_range, next_range;
	size_t data_size;
	...
	do {
		...
		cur_range.low = range.low;
		cur_range.high = range.high;
		while (cur_range.high != 0) {
			ret =
			    gnutls_range_split(session, &cur_range, &cur_range,
					       &next_range);
			if (ret < 0) {
				fail("Error splitting range: %s\n",
				     gnutls_strerror(ret));
				terminate();
			}
			do {
				ret =
				    gnutls_record_send_range2(session, buffer,
							      data_size,
							      &cur_range,
							      &next_range);
			} while (ret == GNUTLS_E_AGAIN
			...
			total += ret;
			data_size -= ret;
			cur_range.low = next_range.low;
			cur_range.high = next_range.high;
		}
		if (ret < 0) {
			fail("Error sending packet: %s\n",
			     gnutls_strerror(ret));
