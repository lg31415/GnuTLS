...
/* Tests whether sending two OCSP responses by a server are received by
 * the client */
...
void doit(void)
{
	...
	char certname[TMPNAME_SIZE], ocspname1[TMPNAME_SIZE], ocspname2[TMPNAME_SIZE];
	FILE *fp;
	...
	ret = gnutls_certificate_set_ocsp_status_request_file(xcred, ocspfile2, 0);
	if (ret < 0)
		fail("ocsp file set failed: %s\n", gnutls_strerror(ret));

	fp = fopen(ocspfile2, "w");
	if (fp == NULL)
		fail("error in fopen\n");
	...
	test_cli_serv(xcred, clicred, "NORMAL", "localhost6", &ocsp_resp2, check_response, NULL); /* the DNS name of the first cert */
