		...
		gnutls_buffer_st *dst_buf)
{
	...
	if (type != GNUTLS_HANDSHAKE_HELLO_REQUEST) {
		...
		ret =
		    _gnutls_buffer_append_data(dst_buf,
					       dataptr, datalen);
		if (ret < 0)
			return gnutls_assert_val(ret);
	}
		...
		uint8_t * dataptr, uint32_t datalen)
{
	 ...
	 */
	if (type != GNUTLS_HANDSHAKE_HELLO_REQUEST) {
		ret = handshake_hash_add_sent(session, type,
				dataptr, datalen,
		...
		size_t head_skip_bytes, size_t header_length)
{
	...
	if (datasize > hs->committed_bytes) {
		...
		ret = handshake_hash_add_sent(session, hs->type,
				data + offset, to_commit,
				&hs->buf);
		if (ret < 0)
			return gnutls_assert_val(ret);
...
int _gnutls_handshake_msg_copy(struct handshake_msg_st *hs,
		gnutls_buffer_st *dst_buf)
{
	if (!hs || !dst_buf)
		return GNUTLS_E_INTERNAL_ERROR;

	_gnutls_buffer_init(dst_buf);
	return _gnutls_buffer_append_data(dst_buf, hs->buf.data, hs->buf.length);
}
