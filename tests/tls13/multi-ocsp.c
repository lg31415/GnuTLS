...
static ctx_st test1 = {"two responses", ocsp_resp1, 2};
static ctx_st test2 = {"single response", ocsp_resp2, 1};
...
{
	ctx_st *test = ptr;
	unsigned idx = cinfo->cert_index;

	assert(test != NULL);

	if (idx > test->nocsp) {
		myfail("asked for cert index %d, have only %d\n", idx, test->nocsp);
	}

	if (test->ocsp[idx].size == 0) {
		return GNUTLS_E_NO_CERTIFICATE_STATUS;
	}

	ocsp_response->data = gnutls_malloc(test->ocsp[idx].size);
	assert(ocsp_response->data != NULL);

	memcpy(ocsp_response->data, test->ocsp[idx].data, test->ocsp[idx].size);
	ocsp_response->size = test->ocsp[idx].size;
...
void doit(void)
{
	...
	/* set OCSP response1 */
	ret = gnutls_certificate_set_ocsp_status_request_function3(xcred, index1, ocsp_func, &test1, 0);
	if (ret < 0)
		fail("ocsp file set failed: %s\n", gnutls_strerror(ret));

	/* set OCSP response2 */
	ret = gnutls_certificate_set_ocsp_status_request_function3(xcred, index2, ocsp_func, &test2, 0);
	if (ret < 0)
		fail("ocsp file set failed: %s\n", gnutls_strerror(ret));
	...
	test_cli_serv(xcred, clicred, "NORMAL:-VERS-TLS-ALL:+VERS-TLS1.3", "localhost", &test1, check_response, NULL);
	test_cli_serv(xcred, clicred, "NORMAL:-VERS-TLS-ALL:+VERS-TLS1.3", "localhost6", &test2, check_response, NULL);
